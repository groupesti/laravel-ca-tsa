...
- Integrated `CA\Log` (`groupesti/laravel-ca-log`) for structured logging across all TSA service classes.
- `TsaRequestParser`: logs successful request parsing and critical errors during parse failures.
- `TsaResponseBuilder`: logs TSTInfo building, granted response construction, and error response generation.
- `TsaSerialGenerator`: logs serial number generation with bit configuration.
- `TsaServer`: logs incoming requests, successful timestamp issuance, and critical errors for both `RuntimeException` and general `Throwable` failures.
- `TsaVerifier`: logs verification start, completion with result, imprint mismatches, decode failures, TSTInfo extraction, and critical errors.
