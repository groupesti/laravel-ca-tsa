...
class TsaVerifier implements TsaVerifierInterface
{
    ...
    {
        try {
            CaLog::log('tsa_operation', 'info', 'Timestamp verification started: response_size=' . strlen($tsrDer) . ' bytes', [
                'operation' => 'tsa_verify',
                'response_size' => strlen($tsrDer),
                'has_original_data' => $originalData !== null,
            ]);
            ...
            if ($decoded === null || !isset($decoded[0])) {
                CaLog::log('tsa_operation', 'info', 'Timestamp verification failed: unable to decode response', [
                    'operation' => 'tsa_verify',
                    'result' => 'failed',
            ...
            if ($originalData !== null) {
                ...
                if (!hash_equals($expectedHash, $tstInfo['hashedMessage'])) {
                    CaLog::log('tsa_operation', 'info', "Timestamp verification failed: message imprint mismatch for serial={$serial}", [
                        'operation' => 'tsa_verify',
                        'serial_number' => $serial,
                        'result' => 'failed',
                        'reason' => 'imprint_mismatch',
                    ]);
            ...
            CaLog::log('tsa_operation', 'info', "Timestamp verification completed: serial={$serial}, valid=" . ($signatureValid ? 'true' : 'false'), [
                'operation' => 'tsa_verify',
                'serial_number' => $serial,
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'tsa_verify', 'exception' => $e::class]);
    ...
    public function extractTstInfo(string $tsrDer): array
    {
        try {
            ...
            CaLog::log('tsa_operation', 'info', "TSTInfo extracted: serial={$serialNumber}, algorithm={$hashAlgorithm}, policy={$policy}", [
                'operation' => 'tsa_extract_tst_info',
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'tsa_extract_tst_info', 'exception' => $e::class]);
            throw $e;
        }
    }
