...
class TsaServer implements TsaServerInterface
{
    ...
    {
        try {
            CaLog::log('tsa_operation', 'info', 'Timestamp request received: size=' . strlen($tsqDer) . ' bytes', [
                'operation' => 'tsa_handle_request',
                'request_size' => strlen($tsqDer),
            ]);
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'tsa_handle_request', 'exception' => $e::class]);
    ...
    public function createTimestamp(
        ...
        CaLog::log('tsa_operation', 'info', "Timestamp issued: serial={$serialNumberHex}, algorithm={$hashAlgorithm}, policy={$policyOid}", [
            'operation' => 'tsa_create_timestamp',
            'serial_number' => $serialNumberHex,
            'hash_algorithm' => $hashAlgorithm,
            'policy_oid' => $policyOid,
