...
class TsaResponseBuilder
{
    ...
    public function buildTstInfo(
        ...
        CaLog::log('tsa_operation', 'info', "Building TSTInfo: serial={$serialNumberHex}, policy={$policyOid}", [
            'operation' => 'tsa_build_tst_info',
            'serial_number' => $serialNumberHex,
            'policy_oid' => $policyOid,
            'hash_algorithm_oid' => $hashAlgorithmOid,
            'gen_time' => $genTime->format('c'),
            'ordering' => $ordering,
    ...
    public function buildGrantedResponse(
        ...
        try {
            ...
            $response = $this->encodeSequence($statusDer . $contentInfoDer);

            CaLog::log('tsa_operation', 'info', "Timestamp granted response built: algorithm={$hashAlgorithm}, includeCert=" . ($includeCert ? 'true' : 'false'), [
                'operation' => 'tsa_build_granted_response',
            ...
            CaLog::critical($e->getMessage(), ['operation' => 'tsa_build_granted_response', 'exception' => $e::class]);
            throw $e;
        }
    }
    ...
    public function buildErrorResponse(
        ...
        CaLog::log('tsa_operation', 'info', "Timestamp error response built: status={$status}, message=" . ($statusString ?? 'none'), [
            'operation' => 'tsa_build_error_response',
            'status' => $status,
